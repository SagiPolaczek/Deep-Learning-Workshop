...
def generate_feature_distance_ranking(data, method="Pearson"):
    ...
    elif method == "set":  # This is the new set operation to calculate similarity. It does not tolerate all-zero
        # features.
    ...
    # ----------------- SHAKED ADDED ----------------------------- #
        ...
        corr = jsd_matrix(data)
        corr = (
            1 - corr
        )  # TODO: did this row to revert row number 71 and to not change the code too much, maybe should change the code?
...
def generate_matrix_distance_ranking(num_r, num_c, method="Euclidean"):
    ...
    if method == "Euclidean":
        ...
        for i in range(num):
            cord_dist[i, :] = np.abs(coordinate[i, 0] * np.ones(num) - coordinate[:, 0]) + np.abs(
                coordinate[i, 1] * np.ones(num) - coordinate[:, 1]
            )
...
def IGTD_absolute_error(
    ...
    for s in range(max_step):
        ...
        if s > val_step:
            if (
                np.sum(
                    (err_record[-val_step - 1] - np.array(err_record[(-val_step):])) / err_record[-val_step - 1]
                    >= min_gain
                )
                == 0
            ):
    ...
    if save_folder is not None:
        pd.DataFrame(index_record).to_csv(
            save_folder + "/" + file_name + "_index.txt", header=False, index=False, sep="\t", line_terminator="\r\n"
        )
...
def IGTD_square_error(
    ...
    source = source.copy()  # TODO: delete later -> source is ranking_feature matrix
    num = source.shape[0]
    tril_id = np.tril_indices(num, k=-1)
    index = np.array(range(num))
    index_record = np.empty((max_step + 1, num))
...
def table_to_image(
    ...
    print(f"RANKING IMAGE : {ranking_feature.shape}")
    ...
    print(f"COORDINATE : {coordinate[0].shape}")
    print(f"RANKING IMAGE : {ranking_image.shape}")
    print("---------------------")
